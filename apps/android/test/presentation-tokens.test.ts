...
function lintStyle(code: string) {
  return lintV1Source(code).filter(
    ({ code: rule }) => rule === "codewide-presentation(presentation-tokens)",
  );
}
...
describe("V1 presentation contract", () => {
  ...
  it("rejects new raw control heights and radii without restricting canvas dimensions", () => {
    expect(
      lintStyle("StyleSheet.create({ closeButton: { height: 37, borderRadius: 11 } });"),
    ).toHaveLength(2);
    expect(
      lintStyle(
        "StyleSheet.create({ closeButton: { minHeight: controlSize.regular, borderRadius: radii.small }, canvas: { height: 300, width: 400 } });",
      ),
    ).toEqual([]);
  });
  ...
  it("fits both sidebar text lines at maximum accessibility scaling without remeasuring rows", () => {
    const textHeight =
      (typeScale.body.lineHeight + typeScale.label.lineHeight) * APP_MAX_FONT_SIZE_MULTIPLIER;
    ...
    const sectionHeight =
      typeScale.caption.lineHeight * APP_MAX_FONT_SIZE_MULTIPLIER + spacing.xs + spacing.xxs;
  ...
  it.each([
    ...
    "marginTop: -3",
  ])("rejects new ad-hoc style values: %s", (property) => {
    const diagnostics = lintStyle(`StyleSheet.create({ text: { ${property} } });`);
    expect(diagnostics).toHaveLength(1);
    expect(diagnostics[0].code).toBe("codewide-presentation(presentation-tokens)");
  });
  ...
  it("does not impose UI spacing rules on non-style data", () => {
    expect(
      lintStyle(
        "const measurement = { lineHeight: 16, gap: 7 }; const item = <Editor options={{ fontSize: 13 }} />;",
      ),
    ).toEqual([]);
  });
});
