...
const manager = compactSource(readFileSync(new URL("../src/features/ports/PortForwardingManager.tsx", import.meta.url), "utf8"));
...
const ownerPortForwardingContract = compactSource(readFileSync(new URL("../src/features/ports/portForwardingContract.ts", import.meta.url), "utf8"));
const ownerForwardingRow = compactSource(readFileSync(new URL("../src/features/ports/ForwardingRow.tsx", import.meta.url), "utf8"));
const ownerPortListProjection = compactSource(readFileSync(new URL("../src/features/ports/portListProjection.ts", import.meta.url), "utf8"));
...
describe("port forwarding manager", () => {
  ...
  it("carries Doma service identity and automatic policy from the companion", () => {
    ...
    expect(ownerPortForwardingContract).toContain('type ServiceSegment = "active" | "available" | "excluded"');
    expect(manager).toContain("Name, category or port");
  });

  it("keeps unavailable distinct from stopped and transport errors", () => {
    expect(ownerPortForwardingContract).toContain('"stopped" | "connecting" | "live" | "unavailable" | "error"');
