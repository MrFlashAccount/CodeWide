...
const ownerProtocolBlock = compactSource(
  readFileSync(
...
const pendingOwner = compactSource(
  readFileSync(
...
const commandOwner = compactSource(
  readFileSync(
...
const cardOwner = compactSource(
  readFileSync(new URL("../src/features/conversation/turns/Card.tsx", import.meta.url), "utf8"),
);

const unknownOwner = compactSource(
  readFileSync(
    new URL("../src/features/conversation/protocol/UnknownProtocolBlock.tsx", import.meta.url),
    "utf8",
  ),
);

const usageOwner = compactSource(
  readFileSync(
    new URL("../src/features/conversation/protocol/TokenUsageProtocolBlock.tsx", import.meta.url),
    "utf8",
  ),
);

const subagentOwner = compactSource(
  readFileSync(
    new URL(
...
const footerOwner = compactSource(
  readFileSync(
...
const cardStyles = compactSource(
  readFileSync(
    new URL("../src/features/conversation/turns/Card.styles.ts", import.meta.url),
    "utf8",
  ),
);

const bubbleNestedSurfaceStyles = compactSource(
  readFileSync(
    new URL("../src/features/conversation/turns/Card.styles.ts", import.meta.url),
    "utf8",
  ),
...
const diffFileStyles = compactSource(
  readFileSync(
    new URL(
...
const attachmentChipStyles = compactSource(
  readFileSync(
...
const approvalCardStyles = compactSource(
  readFileSync(
    new URL("../src/features/requests/RequestFeature.styles.ts", import.meta.url),
    "utf8",
  ),
);

const agentTurnBody = readFileSync(
...
describe("command activity presentation", () => {
  it("gives an empty authoritative running turn a Thinking bubble, not a floating footer", () => {
    ...
    expect(agentTurnBody).toMatch(
      /!presentation\.hasAgentContent && \(?presentation\.rawTurn\.status === "inProgress"\)? &&/u,
    );
    expect(agentTurnBody).toContain('testID="turn-thinking-placeholder"');
    expect(agentTurnBody).toContain('text="Thinking"');
    expect(sourceHasJsxElement(turn, "Bubble", ['variant="agent"', "footer={"])).toBe(true);
    expect(turn).toContain("renderAgentTurnBody(turn, presentation,");
  ...
  it("renders command input and output as separately copyable sections", () => {
    ...
    expect(ownerProtocolBlock).toContain('block.kind === "commandExecution"');
    expect(ownerProtocolBlock).toContain("<CommandExecutionProtocolBlock");
    expect(commandBlock).toContain(">Input</Text>");
    expect(sourceHasJsxElement(commandBlock, "CopyButton", ["text={command}", "compact"])).toBe(
      true,
    );
    ...
    expect(sourceHasJsxElement(commandBlock, "CopyButton", ["text={body}", "compact"])).toBe(true);
  ...
  it("keeps generic protocol surfaces transparent only when nested in a message bubble", () => {
    ...
    expect(sourceObjectDeclaration(diffFileStyles, "diffFile")).toContain(
      "backgroundColor: colors.code",
    );
    expect(sourceObjectDeclaration(attachmentChipStyles, "attachmentChip")).toContain(
  ...
  it("keeps activity tokens and cost on one text baseline", () => {
    ...
    expect(compactSource(metric)).toContain(
