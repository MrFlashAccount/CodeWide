...
const completeMarkdown = compactSource(
  readFileSync(
...
const completedTurn = compactSource(
  readFileSync(
...
it("invalidates native text display lists, not only the enclosing View", () => {
  const view = readFileSync(
    new URL(
      "../android/app/src/main/java/dev/codewide/app/rendering/NativeStreamingRevealView.kt",
      import.meta.url,
    ),
    "utf8",
  );
