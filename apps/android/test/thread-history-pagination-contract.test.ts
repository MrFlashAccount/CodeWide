...
const detailDatabase = readFileSync(
  new URL("../src/data/thread-detail-database.native.ts", import.meta.url),
  "utf8",
);
const detailSqlite = readFileSync(
  new URL("../src/data/thread-detail-sqlite.native.ts", import.meta.url),
  "utf8",
);
const chatModel = readFileSync(
  new URL("../src/data/thread-chat-model.ts", import.meta.url),
  "utf8",
);
const chatWindowHook = readFileSync(
  new URL("../src/data/use-thread-chat-window.ts", import.meta.url),
  "utf8",
);
...
const ownerTimelineViewportState = compactSource(
  readFileSync(
    new URL("../src/features/conversation/timeline/timelineViewport.ts", import.meta.url),
    "utf8",
  ),
);
const ownerTimelineProjection = compactSource(
  readFileSync(
...
const ownerThreadSyncHistory = readFileSync(
  new URL("../src/data/thread-sync-history.ts", import.meta.url),
  "utf8",
);
const ownerThreadSyncRuntime = readFileSync(
  new URL("../src/data/thread-sync-runtime.ts", import.meta.url),
  "utf8",
);

const ownerThreadSyncRemoteLoader = readFileSync(
  new URL("../src/data/thread-sync-remote-loader.ts", import.meta.url),
  "utf8",
);

const ownerTimelineGestureBindings = compactSource(
  readFileSync(
    new URL("../src/features/conversation/timeline/timelineGestureBindings.ts", import.meta.url),
    "utf8",
  ),
);
const ownerMainConversationHistory = compactSource(
  readFileSync(
...
describe("thread history pagination contract", () => {
  ...
  it("does not advance a backend cursor before its page is durable", () => {
    ...
    expect(ownerThreadSyncHistory).toContain("!persisted.accepted");
    expect(ownerThreadSyncHistory).toContain(
  ...
  it("tops up a short cached head from the canonical fifteen-turn tail", () => {
    ...
    expect(ownerThreadSyncRuntime).toMatch(
      /new ThreadSyncCatchUp\(\s*cached,\s*details\.historyCursor\(connectionId, threadId\)/u,
    );
    expect(remoteWorkspace).not.toContain(
      "new ThreadSyncCatchUp(cached, details.historyCursor(connectionId, threadId) ?? null",
    );
    expect(ownerThreadSyncRemoteLoader).toContain('reason !== "activation"');
    expect(ownerThreadSyncRuntime).toContain(
    ...
    expect(ownerThreadSyncHistory).toMatch(
      /parseThreadTurnsListPage\(\s*await rpcAfterAttach<unknown>/u,
    );
    expect(ownerThreadSyncHistory).toMatch(
      /await details\.mergeTailTurns\(\s*connectionId,\s*threadId,\s*\[\.\.\.page\.turns\]\.reverse\(\),\s*page\.nextCursor,\s*isCurrent,?\s*\)/u,
    );
    expect(detailDatabase).toContain(
  ...
  it("publishes a visible loading state for every explicit range request", () => {
    const loading = historyController.indexOf('status: "loading-history"');
  ...
  it("keeps SQLite residency bounded and derives one atomic ready surface", () => {
    ...
    expect(ownerMainConversationHistory).toContain(
      "const projection = projectThreadChatWindow( chatDatabase, chatWindow,",
    );
    expect(chatModel).toContain("commitRange(");
    expect(ownerMainConversationHistory).toContain(
      "!threadLoadBlocksPresentation(chatSnapshot.status)",
    );
  ...
  it("keeps streaming content subscriptions below the workspace boundary", () => {
    const windowHook = chatWindowHook.slice(
      chatWindowHook.indexOf("export function useThreadChatWindow("),
    );
  ...
  it("does not hold the live projection queue open for the 250ms checkpoint", () => {
    ...
    expect(commit).toContain("options.durable === true");
    expect(commit).toContain("flushPending().catch(() => undefined)");
  });
  ...
  it("preserves optimistic pending rows across an in-flight window installation", () => {
    ...
    expect(installer.indexOf("composeInitialRangeRows(")).toBeLessThan(
      installer.indexOf("const membership = rangeMembership"),
    );
    expect(installer.indexOf("mergePendingTimelineOverlays(")).toBeLessThan(
      installer.indexOf("source.replaceThreadLoaded"),
    );
    expect(installer.indexOf("if (!committed) return")).toBeLessThan(
      installer.indexOf("source.replaceThreadLoaded"),
    );
  });

  it("gives overlapping optimistic mutations independent rollback ownership", () => {
    const staging = detailDatabase;

    expect(staging).toContain("nextStagedPendingOwner++");
  ...
  it("proves cursor-page novelty against durable SQLite instead of the partial hot source", () => {
    ...
    expect(prepend.indexOf("loadDurablePrependRows")).toBeLessThan(
      prepend.indexOf("projectPrependedTurnOrdinals"),
    );
    ...
    expect(prepend.indexOf("await commitThreadProjection({")).toBeLessThan(
      prepend.indexOf("composeExpandedRangeRows("),
    );
  });
  ...
  it("anchors authoritative pages in durable SQLite before projecting their epoch", () => {
    ...
    expect(publish.indexOf("await loadDurableAuthoritativeRows")).toBeLessThan(
      publish.indexOf("projectAuthoritativeHistoryEpoch"),
    );
  });
  ...
  it("keeps three pages resident and trims the far edge only after the gesture", () => {
    ...
    const momentumBegin = ownerTimelineGestureBindings.match(
  ...
  it("projects delivery state as ordinary chronological timeline rows", () => {
    expect(ownerMainConversationPublication).toContain(
      "searchState === null ? history.projection.timeline",
    );
    expect(ownerTimelineProjection).toContain("modelTimeline !== null");
    expect(ownerTimelineProjection).toContain("return modelTimeline");
  ...
  it("keeps transport cursors behind the history viewport interface", () => {
    ...
    const conversationPane = compactSource(
      readFileSync(
        new URL("../src/features/conversation/conversationCapabilities.ts", import.meta.url),
        "utf8",
      ),
    );
