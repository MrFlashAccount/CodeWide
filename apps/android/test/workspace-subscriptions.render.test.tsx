...
it("updates recording aura without rerendering its workspace children", async () => {
  ...
  const view = render(
    <WorkspaceVoiceAura resources={resources} controller={null}>
      <Workspace />
    </WorkspaceVoiceAura>,
  );
  const baseline = workspaceRender.mock.calls.length;
  await act(async () => {
    voiceInputs.insert(voiceRow("server\u0000other-thread", "recording"));
  });
  await waitFor(() => expect(view.UNSAFE_getByType(VoiceAura).props.phase).toBe("recording"));
  await act(async () => {
    voiceInputs.update("server\u0000other-thread", (row) => {
      row.phase = "idle";
    });
  });
  await waitFor(() => expect(view.UNSAFE_getByType(VoiceAura).props.phase).toBe("idle"));
  await act(async () => {
    voiceInputs.insert(voiceRow("review:other-thread:file:line", "recording"));
  });
  await waitFor(() => {
  ...
  await act(async () => {
    voiceInputs.update("review:other-thread:file:line", (row) => {
      row.phase = "finishing";
    });
  });
  await waitFor(() => expect(view.UNSAFE_getByType(VoiceAura).props.phase).toBe("idle"));
...
it("updates account menus without rerendering their workspace owner", async () => {
  const collection = createCollection(
    localOnlyCollectionOptions<AccountRateLimitsRow, string>({ getKey: (row) => row.id }),
  );
  ...
  function Workspace() {
    workspaceRender();
    return (
      <WorkspaceAccountUsageMenu database={database} servers={[{ id: "server", name: "Server" }]}>
        <Pressable>
          <Text>Menu</Text>
        </Pressable>
  ...
  await waitFor(() =>
    expect(view.UNSAFE_getByType(UsageMenu).props.accountSources[0].rateLimits?.status).toBe(
      "ready",
    ),
  );
  await act(async () => {
    collection.update("server", (row) => {
      row.status = "loading";
    });
  });
  await waitFor(() =>
