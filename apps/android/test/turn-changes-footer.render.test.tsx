...
import * as changes from "../src/rendering/turn-changes";
...
describe("turn changes footer", () => {
  it("opens the shared Changes capability and keeps its icon at metadata scale", () => {
    ...
    const target: TurnChangesTarget = {
...
it("defers diff parsing until press and uses the latest recorded turn", () => {
  const parse = jest.spyOn(changes, "turnChangedFiles");
  const present = jest.fn();
  const target = { connectionId: "server", threadId: "thread", turnId: "first" };
  ...
  try {
    ...
    const next = { ...target, turnId: "next" };
    const nextDiff = diff.replaceAll("file", "latest");
    view.rerender(
      <TurnChangesContext.Provider value={present}>
        <TurnChangesFooter diff={nextDiff} target={next} />
      </TurnChangesContext.Provider>,
    );
    expect(parse).not.toHaveBeenCalled();
    fireEvent.press(view.getByLabelText("Changes in this turn"));
    expect(parse).toHaveBeenCalledTimes(1);
    expect(present).toHaveBeenCalledWith(next, [
      expect.objectContaining({ path: "latest", patch: nextDiff, additions: 1, deletions: 1 }),
    ]);
  } finally {
    parse.mockRestore();
  }
