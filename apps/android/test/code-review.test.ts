...
} from "../src/features/review/editor/editorBridge";
import { serializeCodeReviewAttachment } from "../src/features/review/comments/reviewComment";
...
describe("code review contracts", () => {
  ...
  it("keys immutable documents by content instead of request identity", () => {
    ...
    expect(revision).not.toBe(
      codeReviewDocumentRevision("src/a.ts", "new", [
        { ...patches[0], diff: "@@ -1 +1 @@\n-a\n+b" },
      ]),
    );
  });
  ...
  it("changes the tree revision only when Pierre-visible file metadata changes", () => {
    const files: CodeReviewFileItem[] = [
      { path: "/repo/a.ts", treePath: "a.ts", status: "modified", additions: 1, deletions: 2 },
    ];
    expect(codeReviewWorkspaceRevision(files)).toBe(
      codeReviewWorkspaceRevision(files.map((file) => ({ ...file }))),
    );
    expect(codeReviewWorkspaceRevision(files)).not.toBe(
      codeReviewWorkspaceRevision([{ ...files[0], additions: 3 }]),
    );
  });
});
