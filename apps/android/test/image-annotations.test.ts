...
import { clampNormalizedCoordinate, formatImageAnnotations } from "../src/rendering/image-annotations";

describe("image annotations", () => {
  it("formats stable top-left normalized coordinates for Codex", () => {
    expect(formatImageAnnotations([{
      label: "Photo 1.jpg",
      ...
      "",
      "### Image 1: Photo 1.jpg (/workspace/Photo 1.jpg)",
  ...
  it("drops empty image groups and clamps coordinates", () => {
    expect(formatImageAnnotations([{ label: "unused", reference: null, annotations: [] }])).toBe("");
