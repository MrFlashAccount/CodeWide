...
import { AppPopover } from "../src/ui/AppPopover.android";
...
jest.mock("@expo/ui/jetpack-compose", () => {
  ...
  const Menu = (props: { children: ReactNode; expanded: boolean; cornerRadius: number; onDismissRequest(): void }) =>
...
jest.mock("@expo/ui/jetpack-compose/modifiers", () => ({ width: (value: number) => ({ width: value }) }));
...
it("keeps its anchor mounted and forwards open, dismissal, live data and body actions", () => {
  ...
  function Anchor(props: { onPress?(): void }) {
    useEffect(() => { mounted(); }, []);
    return <Pressable accessibilityLabel="Open usage" onPress={props.onPress}><Text>Usage</Text></Pressable>;
  }
  function Example(props: { value: string; width: number }) {
    const [open, setOpen] = useState(false);
    return <AppPopover open={open} onOpenChange={setOpen} width={props.width} trigger={<Anchor onPress={originalPress} />}>
      <Text>{props.value}</Text>
      <Pressable accessibilityLabel="Body action" onPress={bodyAction}><Text>Expand details</Text></Pressable>
    </AppPopover>;
  }
