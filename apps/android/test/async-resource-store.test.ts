...
describe("async resource identity", () => {
  it("shares a request promise and keeps progressive content visible between revisions", async () => {
    ...
    let calls = 0;
    const first = getAsyncResource(key, 1, async () => { calls += 1; return "first"; }, (value) => value.length, false);
    expect(first.read()).toBe(first.read());
    await expect(first.read()).resolves.toBe("first");
    expect(calls).toBe(1);
    const second = getAsyncResource(key, 2, async () => "first second", (value) => value.length, false, true);
