...
const nativeAuthentication = readFileSync(
  new URL("../src/native/local-authentication.native.ts", import.meta.url),
  "utf8",
);
...
describe("biometric app lock", () => {
  ...
  it("gates the router behind a durable fail-closed preference", () => {
    ...
    const gatedNavigation = rootLayout.indexOf("<Stack screenOptions=", gateStart);
    ...
    expect(gate).toContain("appLockVoiceLifecycle.pauseForAppLock()");
    expect(gate).toContain("appLockVoiceLifecycle.resumeAfterAppUnlock()");
    expect(gate).toContain('result.success && AppState.currentState === "active"');
  });
