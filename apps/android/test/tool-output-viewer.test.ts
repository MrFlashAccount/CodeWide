...
const ownerToolContent = compactSource(readFileSync(new URL("../src/features/conversation/protocol/ToolContent.tsx", import.meta.url), "utf8"));
const ownerFullContentViewer = compactSource(readFileSync(new URL("../src/features/conversation/content/FullContentViewer.tsx", import.meta.url), "utf8"));
const ownerCommandOutput = compactSource(readFileSync(new URL("../src/features/conversation/protocol/CommandOutput.tsx", import.meta.url), "utf8"));

const overlayOwnership = compactSource(readFileSync(new URL("../src/features/conversation/timeline/overlayScrollOwnership.ts", import.meta.url), "utf8"));

const frame = compactSource(readFileSync(new URL("../src/features/conversation/ConversationFrame.tsx", import.meta.url), "utf8"));

const ownerProtocolBodyView = compactSource(readFileSync(new URL("../src/features/conversation/protocol/ProtocolBodyView.tsx", import.meta.url), "utf8"));
const ownerFullContentViewerBody = compactSource(readFileSync(new URL("../src/features/conversation/content/FullContentViewerBody.tsx", import.meta.url), "utf8"));

describe("tool output presentation", () => {
  it("reports collapsed output in lines instead of characters", () => {
    expect(ownerToolContent).toContain('const bodyLines = body === "" ? 0 : body.split("\\n").length;');
    expect(ownerProtocolBodyView).toContain("`Show more · ${bodyLines.toLocaleString()} ${bodyLines === 1 ? \"line\" : \"lines\"}`");
  ...
  it("keeps the fullscreen viewer outside recycled timeline rows", () => {
    const controls = ownerFullContentViewer.slice(ownerFullContentViewer.indexOf("function LargeContentControls"), ownerFullContentViewer.indexOf("function largeContentPresentation"));
    ...
    expect(ownerFullContentViewer).toContain('async (_publish, signal) => await readPrivateAssetText(');
  ...
  it("does not manually reposition the timeline across fullscreen transitions", () => {
    ...
    expect(overlayOwnership.indexOf("dismissComposerKeyboardForOverlay();", freeze)).toBeGreaterThan(freeze);
  ...
  it("keeps the complete chunk and supports native horizontal and vertical scrolling", () => {
    expect(ownerFullContentViewerBody).toContain("variant={selection.presentation === \"terminal\" ? \"terminal\" : \"code\"}");
    expect(ownerFullContentViewerBody).toContain("selection.presentation === \"terminal\" ? stripTerminalControlSequences(selection.text) : selection.text");
