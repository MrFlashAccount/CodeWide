...
describe("checked-in Android project mirrors app config", () => {
  ...
  it("keeps background connection and privacy-safe activity notifications wired", () => {
    ...
    expect(screen).toContain("status: normalizePendingDeliveryState(delivery.state)");
    ...
    expect(screen).toContain("queuedPrompts={projection.queuedPrompts}");
    ...
    expect(screen).toContain('testID="history-loading-indicator"');
    ...
    expect(voiceWorkspace).toContain("createConnectionStateModel()");
    ...
    expect(screen).toContain("connectionId: activeConnectionId");
