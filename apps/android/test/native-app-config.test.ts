...
describe("checked-in Android project mirrors app config", () => {
  ...
  it("keeps async data ownership in resources and event-driven preview controllers", () => {
    ...
    expect(screen).toContain('`thread-hydration:${historyResourceId}:${threadOpenGeneration}:${historyEpoch}:${provisionalThread === null ? "materialized" : "provisional"}`');
    ...
    expect(screen).toContain('`thread-lifecycle-repair:${connectionId}:${threadId}:${staleLifecycleTurnId}`');
  ...
  it("keeps background connection and privacy-safe activity notifications wired", () => {
    ...
    expect(screen).toContain('<ResourceComposerMenu\n        visible={menuVisible}');
    expect(screen).not.toContain('{menuVisible && <ResourceComposerMenu');
    ...
    expect(screen).toContain('const conversationActivity: ConnectionActivity | null = server?.status === "connecting"');
    expect(screen).not.toContain('historyViewport.phase === "loading" || !timelinePositioned ? "updating" : null');
    expect(screen).toContain('status: cachedSnapshotAvailable ? "background-updating" : "initial-loading"');
    expect(screen).toContain('historyViewport.status === "loading-history"');
    expect(screen).toContain('historyViewport.status === "background-retrying"');
    ...
    expect(threadDetailDatabase).toContain("restoreNewerBuffer: Math.floor(request.residentTurnLimit / 2)");
    ...
    expect(screen).toContain("await remote.readThread(connectionId, threadId, null)");
    ...
    expect(screen).toContain('const [timelineDidPosition, setTimelineDidPosition] = useState(false)');
    expect(screen).toContain('<ConversationPane\n        key={navigationKey}');
    expect(screen).toContain('timelineInteractionScopeRef.current === composerScope');
    expect(screen).toContain('if (anchorIndex < 0 && !historyRestoreReady) return;');
    ...
    expect(screen).toContain('pendingOffset <= 1 ? "latest" : "legacy_offset_fallback"');
    ...
    expect(screen).toContain('const pendingRestoreOffsetRef = useRef<number | null>(initialRestoreOffset)');
    ...
    expect(screen).toContain("const chatWindow = useThreadChatWindow(chatDatabase, chatWindowRequest)");
    expect(screen).toContain("residentTurnLimit: historyResourceRaw?.residentTurnLimit ?? THREAD_RESIDENT_TURN_LIMIT");
    expect(screen).toContain("residentMaxOrdinal: historyResourceRaw === null");
    expect(screen).not.toContain("threadInitialWindow");
    expect(screen).not.toContain("activeResidentOffset");
    expect(screen).toContain("anchorTurnId: composerState.historyAnchorTurnId ?? null");
    ...
    expect(screen).toContain('status: provisionalThread !== null ? "ready" : remoteThread === null ? "initial-loading" : "background-updating"');
