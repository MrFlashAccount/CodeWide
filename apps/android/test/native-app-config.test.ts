...
describe("checked-in Android project mirrors app config", () => {
  ...
  it("renders attached Markdown and isolated HTML with the reusable document preview", () => {
    ...
    expect(documentPreviewHost).toContain('await materializePrivateAsset(request.source ?? { kind: "path", path: request.path }, request.getTransferAccess)');
  ...
  it("keeps code review readonly, offline and attached as one structured artifact", () => {
    ...
    expect(screen).toContain("presentThreadChanges(threadResources?.value ?? null, true)");
    ...
    expect(codeReviewWorkspace).toContain("useEffect(() => {");
    expect(codeReviewWorkspace).toContain("void onInitialLoad().then(");
    ...
    expect(screen).toContain('actions={changeScopeMenuActions(availableChangeScopes, effectiveChangeScope)}');
  ...
  it("keeps async data ownership in resources and event-driven preview controllers", () => {
    ...
    expect(screen).toContain("const selectThread = (value: string) => {");
    ...
    expect(imagePreviewHost).toContain("registerAnnotationHandler((text) => handleAnnotation(text), voiceRuntime)");
  });
  ...
  it("keeps a server-scoped new chat local until the first send", () => {
    ...
    expect(threadDetailDatabase).toContain("collection.startSyncImmediate()");
  ...
  it("keeps background connection and privacy-safe activity notifications wired", () => {
    ...
    expect(nativeFrameStore).toContain("PROJECTION_SCHEMA_VERSION = 4");
    ...
    expect(actionMenu).toContain('left: event.nativeEvent.locationX');
    expect(actionMenu).toContain('top: event.nativeEvent.locationY');
    expect(actionMenu).toContain('style={[styles.longPressAnchor, longPressAnchor]}');
    expect(actionMenu).not.toContain('style={StyleSheet.absoluteFill}');
    ...
    expect(heroUIRoot).toContain("<ImagePreviewHost>");
    expect(screen).toContain("useImagePreviewAnnotationHandler(attachImageReview)");
    ...
    expect(imagePreviewHost).toContain("Attach image review");
    expect(imagePreviewHost).toContain("Attach review");
    expect(screen).toContain('`codex-image-review-${new Date().toISOString()');
    ...
    expect(richMarkdown).toContain('document: { minWidth: 0, maxWidth: "100%", gap: 5 }');
    ...
    expect(liveProjection).toContain("streamRepairThreadIds(events, projected.threads)");
    ...
    expect(threadDetailDatabase).toContain("collection.startSyncImmediate()");
    expect(threadDetailDatabase).toContain("const release = controlLease.install({ begin, write, commit })");
    expect(threadDetailDatabase).not.toContain("cleanup: () => { controls = null; }");
    ...
    expect(threadDetailDatabase).toContain('throw new Error("Thread detail database sync controller did not start")');
    ...
    expect(screen.match(/trigger="long-press"/g)).toHaveLength(4);
    ...
    expect(bubble).not.toMatch(/\bwidth\s*:/u);
    ...
    expect(screen).toContain('userMessageContent: { minWidth: 0, maxWidth: "100%", gap: 6 }');
    expect(screen).toContain('userMessageBlock: { minWidth: 0, maxWidth: "100%" }');
    expect(screen).toContain('userMessageTextBlock: { minWidth: 0, maxWidth: "100%" }');
    ...
    expect(screen).toContain('const changesLoadingWithoutContent = changesPending && (!changesReady || (threadResources?.value?.changes.length ?? 0) === 0);');
    expect(screen).toContain('const attachmentsInitialLoading = attachmentsPending && !attachmentsReady;');
    expect(screen).toContain('<ComposerContextCount label="Changes" value={sessionChangeCount} refreshing={changesRefreshing}');
    expect(screen).toContain('<ComposerContextCount label="Attachments" value={sessionAttachmentCount} refreshing={attachmentsRefreshing}');
    expect(screen).toContain('onInitialLoad: () => onLoadThreadResources(changesPreferences.scope ?? undefined, "changes")');
    expect(screen).toContain('onLoadThreadResources?.(undefined, "attachments")');
    expect(screen).toContain('activeThreadResourcesTaskKey === null ? null : "active-thread-resources"');
