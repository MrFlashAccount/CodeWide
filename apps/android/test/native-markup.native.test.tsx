...
import { UserImageTileFrame } from "../src/features/conversation/turns/UserImageTileFrame";
...
} from "../src/rendering/MessageAttachmentTile";
import { MessageFooterRow } from "../src/rendering/MessageFooterRow";
import { controlSize, layoutSize, radii } from "../src/theme";
...
const code = jest.fn((source: string, language: string, path: string) => (
  <Text>{`${language}:${path}:${source}`}</Text>
));
const image = jest.fn((source: string, title: string) => <Text>{`${title}:${source}`}</Text>);
const link = jest.fn((href: string, children: React.ReactNode) => (
  <Text accessibilityLabel={href}>{children}</Text>
));

function view(html: string) {
  return (
    <RichContentWidthProvider width={360}>
      <NativeMarkup html={html} code={code} image={image} link={link} />
...
it("keeps inline media geometry through loading, decode, error and retry", () => {
  ...
  for (const content of [
    ...
    expect(result.getByTestId("inline-media-frame")).toHaveStyle({
      height: 220,
      overflow: "hidden",
    });
  }
...
it("reserves metadata-sized media before loading and changes only with explicit geometry", () => {
  ...
  result.rerender(
    <InlineMediaFrame height={180}>
      <Text>Decoded</Text>
    </InlineMediaFrame>,
  );
  expect(result.getByTestId("inline-media-frame")).toHaveStyle({ height: 180 });
  result.rerender(
    <InlineMediaFrame height={300}>
      <Text>After viewport resize</Text>
    </InlineMediaFrame>,
  );
  expect(result.getByTestId("inline-media-frame")).toHaveStyle({ height: 300 });
});

it("stretches image previews from the content line while images cover them", () => {
  expect(StyleSheet.flatten(userMessageStyles.userImageGallery)).toMatchObject({
    alignSelf: "flex-start",
    maxWidth: "100%",
    width: 320,
  });
  expect(StyleSheet.flatten(userMessageStyles.userImageFill)).toMatchObject({
    alignSelf: "stretch",
    flex: 1,
    height: "100%",
    width: "100%",
  ...
  const result = render(
    <UserImageTileFrame layout="hero">
      <Text>Loading</Text>
    </UserImageTileFrame>,
  );
  ...
  result.rerender(
    <UserImageTileFrame layout="hero">
      <Image source={{ height: 2000, uri: "data:image/png;base64,fixture", width: 1000 }} />
    </UserImageTileFrame>,
  );
  expect(result.getByTestId("user-image-tile-frame")).toHaveStyle({ height: 180, width: "100%" });
  result.rerender(
    <UserImageTileFrame layout="tile">
      <Text>Error</Text>
    </UserImageTileFrame>,
  );
  expect(result.getByTestId("user-image-tile-frame")).toHaveStyle({
...
it("keeps completion metadata and localized time in one footer row", () => {
  const result = render(
    <MessageFooterRow time="1:05 PM">
      <Text>Completed · 2s</Text>
    </MessageFooterRow>,
  );
  expect(result.getByText("Completed · 2s")).toBeTruthy();
  expect(result.getByTestId("turn-footer")).toHaveStyle({
    flexDirection: "row",
    alignItems: "center",
  });
  expect(result.getByTestId("turn-footer-metadata")).toHaveStyle({
    flexShrink: 0,
    flexWrap: "nowrap",
  });
...
it("renders compact file tiles without opening files until pressed", () => {
  const open = jest.fn();
  const names = [
    "long-file-name-with-important-extension.txt",
    "report.md",
    "clip.mp4",
    "data.csv",
    "archive.zip",
  ...
  expect(grid.props.horizontal).toBe(true);
  expect(grid.props.nestedScrollEnabled).toBe(true);
  expect(grid.props.showsHorizontalScrollIndicator).toBe(false);
  const columns = result.getAllByTestId("two-row-column");
  expect(columns).toHaveLength(3);
  expect(columns.map((column) => column.children.length)).toEqual([2, 2, 1]);
...
it("shows only known file size and disables unavailable file actions", () => {
  const result = render(
    <MessageAttachmentTile name="report.txt" label="Text" bytes={2048} icon={null} />,
  );
...
it("renders LaTeX through native SVG and preserves unsupported input", async () => {
  const result = render(
    view(
      "<p>Formula <cw-inline-math>x^2</cw-inline-math></p><cw-display-math>\\unknown{1}</cw-display-math>",
    ),
  );
...
it("renders nested disclosures natively and retains open state during streaming", () => {
  const result = render(
    view(
      "<details><summary>Outer</summary><p>First</p><details><summary>Inner</summary><b>Nested</b></details></details>",
    ),
  );
  ...
  result.rerender(
    view(
      "<details><summary>Outer</summary><p>Updated</p><details><summary>Inner</summary><b>Nested update",
    ),
  );
...
it("reserves only valid intrinsic image dimensions at the capability boundary", () => {
  render(
    view('<img src="a.png" width="640" height="320"><img src="b.png" width="100%" height="320">'),
  );
...
it("reveals complete table rows without replaying existing row identities on append", () => {
  const first =
    '<table><thead><tr><th>Name</th><th>Value</th></tr></thead><tbody><tr><td rowspan="2">Group</td><td>One</td></tr></tbody></table>';
  const next = first.replace("</tbody>", "<tr><td>Two</td></tr></tbody>");
  const result = render(
    <StreamingRevealSurface streamKey="turn:one">{view(first)}</StreamingRevealSurface>,
  );
  ...
  fireEvent(result.getByTestId("markup-table-cell-0-0"), "layout", {
    nativeEvent: { layout: { height: 40 } },
  });
  expect(result.UNSAFE_getAllByType(NativeRevealSurface)[0]?.props.ready).toBe(false);
  fireEvent(result.getByTestId("markup-table-cell-0-1"), "layout", {
    nativeEvent: { layout: { height: 40 } },
  });
  ...
  result.rerender(
    <StreamingRevealSurface streamKey="turn:one">{view(next)}</StreamingRevealSurface>,
  );
  ...
  expect(
    historical.UNSAFE_getAllByType(NativeRevealSurface).every((row) => row.props.animate === false),
  ).toBe(true);
});
...
it("ignores hidden and executable subtrees and document CSS", () => {
  const result = render(
    view(
      '<div style="position:absolute; font-size:900px" onclick="evil()"><script>secret</script><iframe>private</iframe><p hidden>Hidden</p><b>Visible</b></div>',
    ),
  );
...
it("preserves read-only form content and routes media through the file capability", () => {
  const result = render(
    view(
      '<form><label>Example</label><input type="checkbox" checked><textarea>Read only</textarea></form><progress value="3" max="4"></progress><audio src="/private/clip.opus" title="Recording"></audio>',
    ),
  );
...
it("places table spans without overlap and sizes rows from their contents", () => {
  const table = findTag(
    engine.buildTTree(
      '<table><tr><td rowspan="2">A</td><td colspan="2">B</td></tr><tr><td>C</td><td>D</td></tr></table>',
    ),
    "table",
  );
  ...
  expect(grid.cells.map((cell) => [cell.row, cell.column, cell.rowSpan, cell.colSpan])).toEqual([
    [0, 0, 2, 1],
    [0, 1, 1, 2],
    [1, 1, 1, 1],
    [1, 2, 1, 1],
  ...
  const result = render(
    view("<table><tr><th>Head</th><th>Other</th></tr><tr><td>Left</td><td>Right</td></tr></table>"),
  );
...
it("treats zero rowspan as remaining rows and normalizes invalid spans", () => {
  const table = findTag(
    engine.buildTTree(
      '<table><tr><td rowspan="0">A</td><td colspan="-2">B</td></tr><tr><td>C</td></tr></table>',
    ),
    "table",
  );
  if (table === undefined) throw new Error("Expected table");
...
it("limits zero rowspan to its row group and finds room for an entire colspan", () => {
  const table = findTag(
    engine.buildTTree(
      '<table><tbody><tr><td>A</td><td rowspan="0">B</td></tr><tr><td colspan="2">C</td></tr></tbody><tbody><tr><td>D</td></tr></tbody></table>',
    ),
    "table",
  );
  if (table === undefined) throw new Error("Expected table");
