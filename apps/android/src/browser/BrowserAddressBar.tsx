...
type AddressEdit = { readonly status: "view" } | { readonly status: "edit"; readonly text: string; readonly error: string | null };
...
export function BrowserAddressBar(props: BrowserAddressBarProps) {
  ...
  >
    <View style={[styles.row, edit.status === "edit" && styles.rowEditing]}>
      <AppTextInput ref={input} accessibilityLabel="Browser address" voiceInput={false} value={edit.status === "edit" ? edit.text : props.url}
        onFocus={focus} onChangeText={change} onSubmitEditing={submit} keyboardType="url" returnKeyType="go"
        autoCapitalize="none" autoCorrect={false} selectTextOnFocus multiline={false} placeholder="Enter address" style={styles.input} />
      {edit.status === "edit" && <Pressable accessibilityRole="button" accessibilityLabel="Cancel address editing" onPress={cancel} style={styles.button}>
        <Ionicons name="close" size={iconSize.action} color={colors.textMuted} />
      </Pressable>}
      {edit.status === "edit" && <Pressable accessibilityRole="button" accessibilityLabel="Go to address" onPress={submit} style={styles.button}>
        <Ionicons name="arrow-forward" size={iconSize.action} color={colors.text} />
      </Pressable>}
    </View>
    {edit.status === "edit" && edit.error !== null && <AppText accessibilityRole="alert" style={styles.error}>{edit.error}</AppText>}
  </Reanimated.View>;
}
...
const styles = StyleSheet.create({
  ...
  row: { height: controlSize.regular, flexDirection: "row", alignItems: "center", borderRadius: radii.medium, borderWidth: StyleSheet.hairlineWidth, borderColor: "transparent", backgroundColor: colors.background },
  rowEditing: { borderColor: colors.border, backgroundColor: colors.surfaceRaised },
  input: { flex: 1, minWidth: 0, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, color: colors.text, ...typeScale.label },
  button: { width: controlSize.regular, height: controlSize.regular, flexShrink: 0, alignItems: "center", justifyContent: "center" },
