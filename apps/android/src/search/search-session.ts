...
export class SearchSession {
  ...
  readonly filters$ = observable<SearchFilterValue>({ serverId: "", threadId: "", project: "", from: "", until: "" });
  ...
  changeText(text: string): void {
    this.text$.set(text);
    this.cancelPending();
    if (text.trim() === "") { this.request$.set(null); this.scrollOffset = 0; return; }
    this.timer = setTimeout(() => { this.timer = null; this.submit(); }, 250);
  }
  ...
  submit(): boolean {
    ...
    try {
      ...
      if (from !== null && until !== null && from >= until) throw new Error("The start date must precede the end date");
    } catch (cause) {
    ...
    this.error$.set(null);
    this.scrollOffset = 0;
    this.request$.set({ ...filters, text, page: 0, revision: (this.request$.peek()?.revision ?? 0) + 1 });
    return true;
  }
  ...
  rememberScroll(offset: number): void { this.scrollOffset = Math.max(0, offset); }
  shouldFocus(): boolean { return this.focusRequested; }
  requestFocus(): void { this.focusRequested = true; }
  didFocus(): void { this.focusRequested = false; }
}
