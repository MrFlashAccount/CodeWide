...
export function createThreadResourceLoader({
  ...
  const loadThreadResources = async (
    ...
    if (conflict !== undefined) {
      ...
      return loadThreadResources(connectionId, threadId, scope, kind);
    }
  ...
  return { loadThreadChangeDiff, loadThreadResources };
}
...
function initializedThreadResourceKinds(
  row:
    | { readyKinds?: readonly ThreadResourceKind[]; value: ThreadResourcesValue | null }
    | undefined,
): readonly ThreadResourceKind[] {
  if (row?.readyKinds !== undefined) {
    return row.readyKinds;
  }
  return row?.value === null || row?.value === undefined ? [] : ["changes", "attachments"];
}
