...
type ArtifactReference =
  ...
  | { readonly codewideAsset: { readonly version: 1; readonly id: string; readonly byteLength: number; readonly contentType: string } }
  | { readonly uri: string };
...
export function compactTurnArtifactReferences(turn: unknown): readonly ArtifactReference[] {
  ...
  const append = (value: unknown): void => {
    ...
    const key = "savedPath" in reference ? `path:${reference.savedPath}`
      : "uri" in reference ? `uri:${reference.uri}` : `content:${reference.codewideAsset.id}`;
  ...
  for (const item of turn.items) {
    ...
    for (const content of [item.content, item.contentItems, item.output, isRecord(item.result) ? item.result.content : null]) {
...
function artifactReference(value: unknown): ArtifactReference | null {
  ...
  if (typeof value.uri === "string" && value.uri.startsWith("sandbox:") && absolutePath(value.uri.slice(8))) {
