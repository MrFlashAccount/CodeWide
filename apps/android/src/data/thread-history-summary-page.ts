  ...
  switch (value.type) {
    case "text":
      return typeof value.text === "string" && Array.isArray(value.text_elements)
        && value.text_elements.every((element: unknown) => isRecord(element)
...
function isSummaryItem(value: unknown): boolean {
  ...
  switch (value.type) {
    case "userMessage":
      return (value.clientId === undefined || value.clientId === null || isId(value.clientId))
        && Array.isArray(value.content) && value.content.every(isUserInput);
    case "agentMessage":
      return typeof value.text === "string"
        && (value.phase === undefined || value.phase === null
...
function isSummaryTurn(value: unknown): boolean {
  if (!isRecord(value) || !isId(value.id)
    ...
    || !isOptionalTime(value.startedAt) || !isOptionalTime(value.completedAt) || !isOptionalTime(value.durationMs)
    || (value.error !== undefined && value.error !== null
      && (!isRecord(value.error) || typeof value.error.message !== "string"))) return false;
  if (value.items === undefined) {
    ...
    return content?.version === 1 && whole !== null
      && typeof whole.id === "string" && /^[a-f0-9]{64}$/u.test(whole.id)
      && typeof whole.byteLength === "number" && Number.isSafeInteger(whole.byteLength) && whole.byteLength >= 0
  ...
  return value.itemsView === undefined || value.itemsView === "summary" || value.itemsView === "notLoaded";
}
...
export function parseThreadHistorySummaryPage(
  ...
  if (!isRecord(value) || !Array.isArray(value.data) || typeof value.hasMore !== "boolean"
    ...
    || value.data.length > requestedLimit || (value.hasMore && value.data.length === 0)
    || !value.data.every(isSummaryTurn)) throw new Error("Companion returned an invalid history summary page");
