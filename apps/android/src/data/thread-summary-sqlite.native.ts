...
export function createThreadSummarySqlite(): ThreadSummarySqlite {
  ...
  const scheduleCheckpoint = (): void => {
    ...
    checkpointTimer = setTimeout(() => {
      ...
      void flushPending().catch((cause: unknown) =>
        console.warn("Thread summary SQLite checkpoint failed", cause),
      );
    }, CHECKPOINT_DELAY_MS);
  };
  ...
  const readRows = async (
    ...
    await read(async (executor) => await executeRows(executor, sql, params));
  ...
  return {
    ...
    async loadConnectionRows(connectionId) {
      return await readRows(`SELECT __payload FROM ${TABLE} WHERE connection_id = ?`, [
        connectionId,
      ]);
    },
    async loadUnread() {
