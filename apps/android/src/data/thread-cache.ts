  ...
  for (let turnIndex = thread.turns.length - 1; turnIndex >= 0; turnIndex -= 1) {
    ...
    for (let itemIndex = items.length - 1; itemIndex >= 0; itemIndex -= 1) {
      ...
      if (item?.type === "agentMessage") {
        if (turn.status === "inProgress" && item.phase === "final_answer") {
          continue;
        }
        const text = compactPreview(item.text);
        if (text !== "") {
      ...
      if (item?.type === "userMessage") {
        ...
        if (item.content.some((part) => part.type === "skill")) {
          return "Skill";
        }
      }
    }
...
export function plainThreadPreview(value: string): string {
  return value
    .replaceAll(/!\[([^\]]*)\]\([^)]*\)/gu, (_match, alt: string) =>
      alt.trim() === "" ? "Photo" : alt,
    )
    .replaceAll(/\[([^\]]+)\]\([^)]*\)/gu, "$1")
    .replaceAll(/^\s{0,3}(?:#{1,6}\s+|>\s*|[-+*]\s+|\d+[.)]\s+)/gmu, "")
    .replaceAll(/```[^\n]*\n?|```/gu, " ")
    .replaceAll(/[*_~`]+/gu, "")
    .replaceAll(/<[^>]+>/gu, " ")
    .replaceAll(/\s+/gu, " ")
...
export function serializeThreadForCache(thread: Thread): string {
  ...
  if (emptyRaw.length > MAX_CACHED_THREAD_JSON_CHARS) {
    return emptyRaw;
  }
