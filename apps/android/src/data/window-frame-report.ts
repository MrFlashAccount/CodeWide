...
export function parseWindowFrameSample(input: unknown): WindowFrameSample | null {
  ...
  if (activity !== "scroll" && activity !== "other") {
    return null;
  }
  const appBuild = input.appBuild ?? null;
  if (
    appBuild !== null &&
    (typeof appBuild !== "number" || !Number.isSafeInteger(appBuild) || appBuild < 0)
  ) {
    return null;
  }
  ...
  if (
    Number(input.windowEndUnixMs) < Number(input.windowStartUnixMs) ||
    Number(input.jankFrameCount) > Number(input.frameCount)
  ) {
    return null;
  }
  return { activity, appBuild, surface, values };
}
...
export function parseWindowFrameReport(json: unknown): WindowFrameReport | null {
  if (typeof json !== "string" || json.length > 1_048_576) {
    return null;
  }
  let input: unknown;
  try {
  ...
  if (!record(input) || input.version !== 1 || !Array.isArray(input.windows)) {
    return null;
  }
  if (
    typeof input.evictedWindows !== "number" ||
    !Number.isSafeInteger(input.evictedWindows) ||
    input.evictedWindows < 0
  ) {
    return null;
  }
  if (
    typeof input.unobservedWindows !== "number" ||
    !Number.isSafeInteger(input.unobservedWindows) ||
    input.unobservedWindows < 0
  ) {
    return null;
  }
  ...
  return {
    evictedWindows: input.evictedWindows,
    unobservedWindows: input.unobservedWindows,
    version: 1,
    windows,
  };
}
