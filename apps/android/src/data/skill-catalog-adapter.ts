...
import type { CatalogSkill, InstalledSkillPlugin, SkillPlugin, SkillSource } from "./skill-catalog-types";
...
function source(value: unknown): SkillSource | null {
  return value === "repo" || value === "user" || value === "system" || value === "admin" ? value : null;
}
...
function icon(metadata: Record<string, unknown> | null): PrivateAssetSource | null {
  for (const key of ["logoDark", "composerIcon", "logo"]) {
    ...
    if (path !== null && path.startsWith("/") && !path.includes("\0")) return { kind: "path", path };
  }
  for (const key of ["logoUrlDark", "composerIconUrl", "logoUrl"]) {
    ...
    try {
      ...
      if (parsed.protocol === "https:" && parsed.username === "" && parsed.password === "") return { kind: "remote", url };
    } catch {
...
export function parseCatalogSkills(value: unknown): CatalogSkill[] {
  ...
  for (const entry of entries) {
    ...
    for (const item of items) {
      ...
      if (name === null || path === null || typeof skill?.description !== "string" || typeof skill.enabled !== "boolean") throw new Error("Invalid skill metadata");
      const metadata = record(skill.interface);
      skills.set(path, {
        name, path, description: skill.description, enabled: skill.enabled,
        catalog: {
          title: label(metadata?.displayName) ?? name,
          description: label(metadata?.shortDescription) ?? label(skill.shortDescription) ?? skill.description.trim(),
          source: source(skill.scope), pluginLink: { status: "unavailable" },
        },
      });
    }
  }
...
export function parseInstalledSkillPlugins(value: unknown): InstalledSkillPlugin[] {
  ...
  if (Array.isArray(response.marketplaceLoadErrors) && response.marketplaceLoadErrors.length > 0) throw new Error("Some plugin marketplaces are unavailable");
  ...
  for (const item of response.marketplaces) {
    ...
    if (marketplaceName === null || !Array.isArray(marketplace?.plugins)) throw new Error("Invalid plugin marketplace");
    for (const item of marketplace.plugins) {
      ...
      result.push({
        plugin: { id: `${marketplaceName}:${id}`, label: label(metadata?.displayName) ?? name, icon: icon(metadata) },
        pluginName: name, marketplacePath,
...
export function parsePluginSkillPaths(value: unknown): string[] {
  ...
  return skills.flatMap((item) => {
    const skill = record(item);
    if (skill === null || !(skill.path === null || typeof skill.path === "string")) throw new Error("Invalid plugin skill path");
    return typeof skill.path === "string" && skill.path !== "" ? [skill.path] : [];
  });
}

export function assignSkillPlugins(skills: CatalogSkill[], plugins: ReadonlyMap<string, SkillPlugin>, complete: boolean): CatalogSkill[] {
  return skills.map((skill) => {
    if (skill.catalog === undefined) return skill;
    const plugin = plugins.get(skill.path) ?? null;
    return { ...skill, catalog: { ...skill.catalog, pluginLink: plugin !== null || complete
