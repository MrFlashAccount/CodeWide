  ...
  return (repairForegroundConnection: (connectionId: string) => Promise<void>): void => {
    const repairCatalogs = (): void => {
      for (const connectionId of enabledConnectionIds()) {
        ...
        void refreshThreadCatalog(connectionId).catch((cause: unknown) => {
    ...
    const wakeFocusedConnections = (): void => {
      ...
      for (const connectionId of enabledConnectionIds()) wakeNativeConnection(connectionId);
    };
    for (const subscription of catalogLifecycleSubscriptions) subscription.remove();
    ...
    if (catalogRepairTimer !== null) clearInterval(catalogRepairTimer);
