...
export function AccountUsageRow({ testID, label, plan, status, reset, children }: AccountUsageRowProps) {
  return <View testID={testID} style={styles.row}>
    <View accessible accessibilityLabel={`Account ${status}`} style={[styles.dot, { backgroundColor: status === "exhausted" ? colors.red : status === "active" ? colors.green : colors.textDim }]} />
    <View style={styles.content}>
      <View style={styles.titleRow}>
        <Text numberOfLines={1} style={styles.name}>{label}</Text>
        {children}
      </View>
      <View accessible style={styles.metadataRow} accessibilityLabel={`${plan}${reset === null ? "" : ` · Resets ${reset.absolute}${reset.relative === null ? "" : ` · ${reset.relative}`}`}`}>
        <Text numberOfLines={1} style={styles.metadata}>{plan}</Text>
        {reset !== null && <>
          <Text style={styles.metadata}>·</Text>
          <Ionicons name="refresh-outline" size={iconSize.indicator} color={colors.textDim} />
          <Text numberOfLines={1} style={[styles.metadata, styles.date]}>{reset.absolute}</Text>
          {reset.relative !== null && <Text numberOfLines={1} style={styles.metadata}>· {reset.relative}</Text>}
        </>}
      </View>
    </View>
  </View>;
...
const styles = StyleSheet.create({
  row: { flexDirection: "row", alignItems: "flex-start", gap: spacing.xs, paddingVertical: spacing.xs },
