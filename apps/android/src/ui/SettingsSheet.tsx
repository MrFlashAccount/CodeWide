...
import { listRowHeight, listRowPosition } from "./AppListRow.types";
...
export function SettingsSheet(props: SettingsSheetProps) {
  ...
  const selectedServer = page.kind === "server"
    ? props.servers.find((server) => server.id === page.id)
    : undefined;
  // A removed server returns to the overview without retaining its editor or account data.
  const overview = page.kind === "overview" || (page.kind === "server" && selectedServer === undefined);
  const title = overview ? "Settings" : page.kind === "advanced" ? "Advanced" : selectedServer?.title ?? "Settings";
  ...
  const changeOpen = useEvent((open: boolean) => { if (!open) props.onClose(); });

  return (
    <AppSheet
      isOpen
      onOpenChange={changeOpen}
      contentProps={{ dismissLabel: "Close settings", performanceSurface: "settings", index: 0, snapPoints: ["65%", "90%"], enableDynamicSizing: false, enableOverDrag: false, contentContainerClassName: "h-full" }}
    >
      <View style={styles.header}>
        {!overview && (
          <Pressable accessibilityRole="button" accessibilityLabel="Back to settings" onPress={back} style={styles.back}>
      ...
      <AppSheetScrollView key={overview ? "overview" : page.kind === "advanced" ? "advanced" : selectedServer?.id} style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        {overview ? (
          <>
            <SettingsSection title="Servers">
              ...
              {props.servers.map((server, index) => (
                <AppListRow
                  ...
                  trailingIcon={{ name: "chevron-forward", size: iconSize.inline, color: colors.textMuted }}
              ...
              <AppListRow title="Add server" fixedHeight={listRowHeight.single} position={props.servers.length === 0 ? "only" : "last"} onPress={props.onAddServer}
                leadingIcon={{ name: "add", size: iconSize.action, color: colors.textMuted }} />
            </SettingsSection>
            {props.security === null ? null : <SettingsSection title="Security">{props.security}</SettingsSection>}
            <AppListRow title="Advanced" description="Interface, experiments and diagnostics" fixedHeight={listRowHeight.double} onPress={openAdvanced}
              leadingIcon={{ name: "options-outline", size: iconSize.action, color: colors.textMuted }}
              trailingIcon={{ name: "chevron-forward", size: iconSize.inline, color: colors.textMuted }} />
            {props.version}
          </>
        ) : page.kind === "advanced" ? props.advanced : selectedServer?.content}
      </AppSheetScrollView>
    </AppSheet>
  );
}
...
const styles = StyleSheet.create({
  header: { minHeight: touchTarget, marginBottom: spacing.xs, flexDirection: "row", alignItems: "center", gap: spacing.compact },
  ...
  sectionTitle: { color: colors.textMuted, ...typeScale.label, fontWeight: typeWeight.semibold, textTransform: "uppercase" },
