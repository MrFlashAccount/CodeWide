...
export type SkillPickerRowModel =
  | { count: number; key: string; kind: "header"; plugin: SkillPlugin | null; title: string }
  | {
      description: string;
      first: boolean;
      key: string;
...
export function skillPickerRows(
  ...
  const groups = new Map<
    string,
    { plugin: SkillPlugin | null; skills: CatalogSkill[]; title: string }
  >();
  const seen = new Set<string>();
  for (const skill of skills) {
    if (!skill.enabled || seen.has(skill.path)) {
      continue;
    }
    seen.add(skill.path);
    if (filter !== "all" && filter !== (skill.catalog?.source ?? "unknown")) {
      continue;
    }
  ...
  for (const [id, group] of [...groups].sort(([, left], [, right]) => {
    if (left.plugin === null && right.plugin !== null) {
      return 1;
    }
    if (left.plugin !== null && right.plugin === null) {
      return -1;
    ...
    group.skills.sort((left, right) => {
      const titleOrder = (left.catalog?.title ?? left.name).localeCompare(
        right.catalog?.title ?? right.name,
      );
      return titleOrder !== 0 ? titleOrder : left.path.localeCompare(right.path);
    });
