...
interface SettingsVersionProps { readonly version: string }

export function SettingsVersion(props: SettingsVersionProps) {
  const label = `Version ${props.version}`;
  const copy = () => { void Clipboard.setStringAsync(label); };
  return (
    <AppText
...
const styles = StyleSheet.create({
  version: { color: colors.textDim, ...typeScale.label, textAlign: "center", paddingTop: spacing.lg, paddingBottom: spacing.sm },
});
