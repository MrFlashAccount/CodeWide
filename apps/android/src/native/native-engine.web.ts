...
class WebNativeEngineSession implements RpcClient {
  ...
  async rpc<T>(_method: string, _params: unknown): Promise<T> { throw new Error("Native remote engine is available on Android only"); }
  async respondToServerRequest(_id: string | number, _result: unknown): Promise<void> { throw new Error("Native remote engine is available on Android only"); }
  stop(): void {}
}
