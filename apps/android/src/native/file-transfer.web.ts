...
export function selectedUploadUri(_upload: SelectedUpload): string | null { return null; }
...
export async function openDownloadedFile(_uri: string, _mimeType?: string): Promise<void> { throw new Error("Opening downloaded files is available in the Android build"); }
export function startUpload(_getAccess: GetTransferAccess, _file: SelectedUpload, _rootId: string, _remotePath: string, _overwrite: boolean, _onProgress: (progress: TransferProgress) => void): RunningTransfer { throw new Error("File transfer is available in the Android build"); }
export function startDownload(_getAccess: GetTransferAccess, _directory: SelectedDirectory, _rootId: string, _remotePath: string, _onProgress: (progress: TransferProgress) => void): RunningTransfer { throw new Error("File transfer is available in the Android build"); }
export function startPreviewDownload(_getAccess: GetTransferAccess, _directory: SelectedDirectory, _absolutePath: string, _onProgress: (progress: TransferProgress) => void): RunningTransfer { throw new Error("File transfer is available in the Android build"); }
