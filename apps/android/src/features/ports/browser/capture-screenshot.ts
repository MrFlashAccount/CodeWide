...
export async function captureBrowserScreenshot(websocketUrl: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      fail(new Error("Screenshot capture timed out"));
    }, 5000);
    socket.onopen = () => {
      try {
        socket.send(
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (!isRecord(response) || response.id !== 1) {
          return;
        }
        if (
    ...
    socket.onerror = () => {
      fail(new Error("Screenshot connection failed"));
    };
    socket.onclose = () => {
      fail(new Error("Screenshot connection closed"));
    };
  });
}
