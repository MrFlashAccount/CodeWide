...
export function codeReviewCommentKey(reference: CodeReviewLineReference): string {
  return `${reference.path}\u0000${reference.coordinate ?? "file"}\u0000${reference.side}\u0000${String(reference.line)}`;
}
...
export function matchesCodeReviewInput(
  ...
  if (expected === null || candidate === null || typeof candidate !== "object") {
    return false;
  }
  return (
    "path" in candidate &&
...
export function serializeCodeReviewAttachment(comments: readonly CodeReviewComment[]): string {
  const ordered = [...comments].sort((left, right) => {
    ...
    return lineOrder !== 0 ? lineOrder : left.createdAt - right.createdAt;
  });
  const lines = [
    "---",
    "kind: codewide-code-review",
    "version: 1",
    `comments: ${String(ordered.length)}`,
    "---",
    "",
  ...
  for (const comment of ordered) {
    lines.push(
      `## \`${escapeInlineCode(comment.path)}\` · ${comment.coordinate === "diff" ? "diff" : comment.side === "old" ? "old line" : "line"} ${String(comment.line)}`,
      "",
