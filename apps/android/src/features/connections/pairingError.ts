export function humanPairingError(cause: unknown): string {
  ...
  if (normalized.includes("wss") || normalized.includes("endpoint"))
    return "This host address is not a valid secure CodeWide endpoint.";
  if (normalized.includes("token") || normalized.includes("pairing"))
    return "This connection code is invalid or has already been used.";
  return message;
}
