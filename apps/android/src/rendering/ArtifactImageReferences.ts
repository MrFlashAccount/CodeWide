...
export function artifactImageReferences(attachments: readonly UserMessageAttachment[]): ReadonlySet<string> {
  ...
  for (const attachment of attachments) {
    ...
    else if (attachment.source.type === "content") references.add(`content:${attachment.source.asset.id}`);
  }
  return references;
}
