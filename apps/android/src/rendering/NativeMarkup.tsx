...
interface NativeMarkupProps extends MarkupCapabilities {
...
export function NativeMarkup(props: NativeMarkupProps): ReactNode {
  ...
  return (
    <Capabilities.Provider value={props}>
      <RenderHTML
        {...nativeMarkupConfig}
...
function PreRenderer(props: CustomRendererProps<TBlock>): ReactNode {
  ...
  return capabilities.code(
    nodeText(props.tnode).replace(/\n$/u, ""),
    language,
    markupNodePath(props.tnode),
  );
}

function ImageRenderer(props: MixedProps): ReactNode {
  return useCapabilities().image(
    props.tnode.attributes.src ?? "",
    props.tnode.attributes.alt ?? "Image",
    markupImageDimensions(props.tnode.attributes),
  );
}

function LinkRenderer(props: MixedProps): ReactNode {
  return useCapabilities().link(
    props.tnode.attributes.href ?? "",
...
function MediaRenderer(props: MixedProps): ReactNode {
  ...
  const source =
    dom !== null && isDomElement(dom)
      ? dom.children.find((node) => isDomElement(node) && node.name === "source")
      : undefined;
  const href =
    props.tnode.attributes.src ??
    (source !== undefined && isDomElement(source) ? source.attribs.src : "") ??
    "";
      ...
      {expanded && (
        <View style={styles.detailsBody}>
          <RichContentWidthProvider
            width={available === null ? null : Math.max(1, available - spacing.sm * 2)}
          >
            <TChildrenRenderer tchildren={body} />
          </RichContentWidthProvider>
        </View>
      )}
    </View>
