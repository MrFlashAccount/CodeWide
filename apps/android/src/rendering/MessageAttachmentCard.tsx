...
export function MessageAttachmentCard(props: MessageAttachmentCardProps) {
  ...
  const open = () => {
    ...
    if (video) { openVideo({ name: attachment.name, source, getAccess }); return; }
    const openPreview = kind === "text" && openCodeDocument !== null ? openCodeDocument : openDocument;
    openPreview({ kind, name: attachment.name, path: attachment.source.type === "path" || attachment.source.type === "scoped" ? attachment.source.path : attachment.name, source, getTransferAccess: getAccess });
  };
  return <MessageAttachmentTile name={attachment.name} label={video ? "Video" : attachment.kind === "audio" ? "Audio" : kind === "download" ? "File" : kind}
    icon={<Ionicons name={video ? "play-circle-outline" : attachment.kind === "audio" ? "musical-note-outline" : "document-text-outline"} size={iconSize.action} color={colors.accent} />}
    {...(attachment.source.type === "content" ? { bytes: attachment.source.asset.byteLength } : {})}
    {...(getAccess === undefined ? {} : { onOpen: open })}
  />;
}

export function attachmentPrivateSource(attachment: UserMessageAttachment): PrivateAssetSource {
