...
import { markdownDocumentBlocks, markdownDocumentTargetIndex, type MarkdownDocumentBlock } from "./markdown-document-blocks";
...
export function MarkdownDocumentView({ segments, target, reviewTarget, maxWidth, textScale, onScroll, footer }: {
  ...
  const contentWidth = Math.max(1, Math.min(viewportWidth, maxWidth ?? viewportWidth) - spacing.md * 2);
  ...
  return (
    <View style={styles.root} onLayout={({ nativeEvent }) => setViewportWidth(Math.floor(nativeEvent.layout.width))}>
      {viewportWidth > 0 && (
          ...
          <LegendList
            ...
            renderItem={({ item }) => (
              <View style={[styles.block, maxWidth === undefined ? null : { maxWidth }]}>
                <RichMarkdownDocumentBlockView block={item} {...(reviewTarget === undefined ? {} : { reviewTarget })} />
              </View>
            )}
            ...
            ListFooterComponent={<View style={[styles.block, maxWidth === undefined ? null : { maxWidth }]}>{footer}</View>}
            style={styles.root}
          />
...
const styles = StyleSheet.create({
  ...
  block: { width: "100%", alignSelf: "center", paddingHorizontal: spacing.md, paddingBottom: spacing.sm },
});
