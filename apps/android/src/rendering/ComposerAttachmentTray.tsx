...
import { composerAttachmentPreviewKind, composerAttachmentSource } from "./composer-attachment-preview";
...
export function ComposerAttachmentTray(props: ComposerAttachmentTrayProps) {
  ...
  return (
    ...
    <ScrollView testID="composer-attachment-strip" horizontal showsHorizontalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
      {props.attachments.filter((item) => !managed.has(item.id)).map((attachment) => <ComposerCard key={attachment.id} attachment={attachment} upload={null} owner={props} />)}
      {uploads.map((upload) => <ComposerCard key={upload.attachment.id} attachment={upload.attachment} upload={upload} owner={props} />)}
    </ScrollView>
...
function ComposerCard(props: ComposerCardProps) {
  ...
  const openFile = documentKind === "text" && openCodeDocument !== null ? openCodeDocument : openDocument;
  ...
  const source = attachment.editor === undefined ? baseSource : { ...baseSource, cacheRevision: String(attachment.editor.revision) };
  ...
  const image = usePrivateAssetUri(attachment.kind === "image" && localUri === null && ready ? source : null, attachment.editor?.revision ?? 0, owner.getAccess);
  const uri = localUri ?? image.uri;
  const groupId = `composer:${owner.scope}`;
  const item = { id: attachment.id, label: attachment.name, source: { uri: uri ?? "about:blank" }, reference: `scoped:${attachment.rootId}:${attachment.path}`, order: 0, draft: { scope: owner.scope, attachmentId: attachment.id } };
  ...
  const open = () => {
    ...
    else if (video && (localUri !== null || ready)) openVideo({ name: attachment.name, source: localUri === null ? source : { kind: "direct", uri: localUri }, getAccess: owner.getAccess });
    else if (ready) openFile({ kind: documentKind, name: attachment.name, path: attachment.path, source, getTransferAccess: owner.getAccess });
  };
  return <AttachmentCard
    compact video={video} name={attachment.name} label={video ? "Video" : attachment.editor === undefined ? composerAttachmentPreviewKind(attachment) : "Drawing"}
    ...
    {...(ready || (video || attachment.kind === "image") && uri !== null ? { onOpen: open } : {})}
    onRemove={() => { composerUploads.remove(owner.scope, attachment.id); owner.onRemove(attachment.id); }}
    onRetry={() => composerUploads.retry(owner.scope, attachment.id)}
    onThumbnailError={() => setFailedUri(localUri)}
  />;
}

const styles = StyleSheet.create({ content: { paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, gap: spacing.xs } });
