...
const noManualReactMemoization = {
  meta: {
    type: "problem",
    docs: {
      description: "Reserve render memoization for React Compiler",
    },
    schema: [],
    messages: {
      manualMemoization:
        "Do not use React.{{hook}} in V1. React Compiler owns render memoization; use useEvent or an explicit state/ref/model owner when identity has runtime meaning.",
    },
  },
  create(context) {
    const reactNamespaces = new Set();
    const forbiddenHooks = new Set(["useCallback", "useMemo"]);
    ...
    return {
      ...
      MemberExpression(node) {
        if (
          ...
          !forbiddenHooks.has(node.property.name)
        ) {
          return;
        }
        context.report({
...
module.exports = {
  rules: {
    ...
    "no-manual-react-memoization": noManualReactMemoization,
