...
/// Moves legacy `CodeWide` host state into its branded locations while keeping
...
/// files are missing, empty, or have unsafe permissions after the migration.
pub fn migrate_legacy_installation(
    ...
    validate_private_file(&paths.current_config_root.join("devices.json"))?;
    Ok(report)
}
