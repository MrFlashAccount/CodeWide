...
-> Result<(), Box<dyn Error>> {
    ...
    let now: u64 = SystemTime::now()
        ...
        .as_millis()
        .try_into()?;
    let mut expiring = connect_as_expires(
        &url,
        "expiry-device",
    ...
    let mut fresh = connect_as(&url, "expiry-device", "threads.read,threads.write").await?;
...
-> Result<(), Box<dyn Error>> {
    ...
    let url = format!("ws://{address}/v2/sync");
    let mut client = connect_as(&url, "lag-device", "threads.read,threads.write").await?;
...
-> Result<(), Box<dyn Error>> {
    ...
    let mut original = connect_as(&url, device, "threads.read,threads.write").await?;
    ...
    let mut blocked = connect_as(&url, device, "threads.read,threads.write").await?;
    ...
    let mut still_blocked =
        connect_as(&restarted_url, device, "threads.read,threads.write").await?;
    expect_close_code(&mut still_blocked, 1008).await?;

    source.fail_purge.store(false, Ordering::SeqCst);
    let mut recovered = connect_as(&restarted_url, device, "threads.read,threads.write").await?;
...
-> Result<(), Box<dyn Error>> {
    ...
    let mut client = connect_as(&url, device, "threads.read,threads.write").await?;
    ...
    let mut repaired = connect_as(&url, device, "threads.read,threads.write").await?;
    ...
    let mut cleanup = connect_as(&url, "cleanup-device", "threads.read,threads.write").await?;
