...
describe("Sync V2 generated contract authority", () => {
  it("binds TypeScript runtime validation to the exact executable schema", () => {
    const source = readFileSync(new URL("../../../apps/companion/contract/v2.json", import.meta.url), "utf8");
  ...
  it("rejects omitted nullable keys, unknown fields, and illegal error pairs", () => {
    ...
    expect(() => parseV2ServerFrame(JSON.stringify({
      type: "queryFailed",
      requestId: "request",
  ...
  it("rejects extra outbound fields through the generated client schema", () => {
    expect(() => validateV2ClientFrame({
      type: "command",
      requestId: "request",
      operationId: "operation",
      command: { kind: "thread.delete", threadId: "thread", sourceMethod: "thread/delete" } as never,
    })).toThrow(V2ProtocolValidationError);
  });

  it("enforces schema-derived array and error-message bounds", () => {
    expect(() => validateV2ContractDefinition("command", {
      kind: "turn.steer",
      threadId: "thread",
      turnId: "turn",
  ...
  it("matches the shared cross-runtime integer and strict UTC timestamp boundaries", () => {
    const fixtures = JSON.parse(readFileSync(new URL("./fixtures/v2-validator-boundaries.json", import.meta.url), "utf8")) as Array<{
      name: string;
    ...
    for (const fixture of fixtures) {
      const validate = () => validateV2ContractDefinition(fixture.definition, JSON.parse(fixture.json) as unknown);
  ...
  it("rejects sparse and non-JSON array slots before persistence or outbound serialization", async () => {
    ...
    await expect(session.command("operation", {
      kind: "turn.steer",
      threadId: "thread",
      turnId: "turn",
      input: sparseInput,
...
function collectRelativeGraph(entry: string, files = new Set<string>()): Set<string> {
  ...
  for (const match of source.matchAll(/from\s+["'](\.[^"']+)["']/gu)) {
    ...
    try { collectRelativeGraph(candidate, files); } catch { /* type-only barrel outside the V2 graph is absent by contract */ }
  }
  return files;
}
