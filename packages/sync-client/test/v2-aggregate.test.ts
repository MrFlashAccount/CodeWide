...
describe("Sync V2 one, many, and All projection", () => {
  it("keeps identical thread ids distinct and changing selection does not mutate partitions", async () => {
    ...
    await store.commitSnapshot(savedServerA, snapshot({ active: [thread("same", { title: "A" })] }));
    await store.commitSnapshot(savedServerB, snapshot({
      epochId: "epoch-b",
    ...
    const one = await deriveV2AggregateProjection(store, deletions, [savedServerA, savedServerB], { kind: "selected", savedServerIds: [savedServerA] });
    const many = await deriveV2AggregateProjection(store, deletions, [savedServerA, savedServerB], { kind: "selected", savedServerIds: [savedServerB, savedServerA] });
    const all = await deriveV2AggregateProjection(store, deletions, [savedServerA, savedServerB], { kind: "all" });

    expect(one.threads.map(({ identity }) => identity)).toEqual([{ savedServerId: savedServerA, threadId: "same" }]);
    expect(many.threads.map(({ identity, entry }) => [identity.savedServerId, identity.threadId, entry.thread.title])).toEqual([
      [savedServerB, "same", "B"],
      [savedServerA, "same", "A"],
    ]);
    expect(all.servers.map(({ savedServerId }) => savedServerId)).toEqual([savedServerA, savedServerB]);
  ...
  it("fails aggregate reads closed while saved-server deletion is pending", async () => {
    ...
    await store.commitSnapshot(savedServerB, snapshot({ epochId: "epoch-b", revision: "sync-v2-revision:b", active: [thread("visible")] }));
    await deletions.begin(savedServerA);

    const aggregate = await deriveV2AggregateProjection(store, deletions, [savedServerA, savedServerB], { kind: "all" });
  ...
  it("updates only explicit selection when its saved server is deleted", () => {
    expect(removeSavedServerFromV2Selection(
      { kind: "selected", savedServerIds: [savedServerA, savedServerB] },
      savedServerA,
    )).toEqual({ kind: "selected", savedServerIds: [savedServerB] });
    expect(removeSavedServerFromV2Selection({ kind: "all" }, savedServerA)).toEqual({ kind: "all" });
  });
});

describe("Sync V2 owner routing", () => {
  it("routes equal operation ids and every request class to the owning server", async () => {
    const calls: string[] = [];
    ...
    await router.command({ savedServerId: savedServerA, operationId: "same" }, { kind: "thread.delete", threadId: "thread" });
    await router.command({ savedServerId: savedServerB, operationId: "same" }, { kind: "thread.delete", threadId: "thread" });
    await router.action(savedServerA, {
    ...
    expect(calls).toEqual([
      ...
      `${savedServerA}:action:request.resolve`,
    ]);
  });

  it("fails closed when no session owns the target", async () => {
    const router = new SyncV2SessionRouter(new MemoryV2SavedServerDeletionStore());
    await expect(router.query(savedServerA, { kind: "models.list" })).rejects.toThrow("No live Sync V2 session owns");
  });
  ...
  it("blocks every semantic route while durable deletion is pending", async () => {
    ...
    await deletions.begin(savedServerA);
    await expect(router.query(savedServerA, { kind: "models.list" })).rejects.toThrow("blocked by durable deletion intent");
  });
});
...
function fakeSession(savedServerId: typeof savedServerA, calls: string[]): V2SemanticSession {
  return {
    ...
    command: async (operationId) => {
      ...
      return { type: "commandCompleted" as const, operationId, result: { kind: "thread.delete" as const, threadId: "thread" } };
    },
    action: async (action) => {
      calls.push(`${savedServerId}:action:${action.kind}`);
      return { kind: "request.resolve" as const, requestId: action.requestId, state: "resolved" as const };
    },
  };
}
