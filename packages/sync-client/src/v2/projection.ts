...
import type { V2CatalogScope, V2PendingRequest, V2ProjectionChange, V2SnapshotLimits, V2ThreadSummary, V2ThreadWindow, V2U64 } from "./model";
...
export type V2SemanticInvalidation = Extract<V2ProjectionChange, {
...
export interface V2ProjectionStore {
  ...
  commitSnapshot(savedServerId: V2SavedServerId, snapshot: V2SnapshotFrame, signal?: AbortSignal): Promise<V2Projection | null>;
  applyChange(savedServerId: V2SavedServerId, epochId: string, watermark: V2U64, change: V2ProjectionChange): Promise<void>;
...
export class MemoryV2ProjectionStore implements V2ProjectionStore {
  ...
  async commitSnapshot(savedServerId: V2SavedServerId, snapshot: V2SnapshotFrame, signal?: AbortSignal): Promise<V2Projection | null> {
  ...
  async applyChange(savedServerId: V2SavedServerId, epochId: string, watermark: V2U64, change: V2ProjectionChange): Promise<void> {
    const current = this.#active.get(savedServerId);
    if (current === undefined || current.epochId !== epochId) throw new Error("Sync V2 change does not belong to the active projection generation");
    if (compareV2Watermarks(watermark, current.watermark) <= 0) throw new Error("Sync V2 watermark did not advance");
...
export function buildV2Projection(previous: V2Projection | null, snapshot: V2SnapshotFrame): V2Projection {
  assertSnapshotCoverage(snapshot);
  const entries = new Map<string, V2CatalogEntry>();
  for (const entry of previous?.catalog ?? []) entries.set(entry.thread.id, { thread: entry.thread, coverage: "outsideCurrentScope" });
  for (const thread of [...snapshot.catalog.active, ...snapshot.catalog.archived]) entries.set(thread.id, { thread, coverage: "current" });
  ...
  for (const sequenced of snapshot.includedTail) projection = reduceV2Projection(projection, sequenced.watermark, sequenced.change);
  projection.watermark = snapshot.watermark;
  return projection;
}

export function reduceV2Projection(projection: V2Projection, watermark: V2U64, change: V2ProjectionChange): V2Projection {
  const next = clone(projection);
  next.watermark = watermark;
  if (change.kind === "threadUpserted") {
    upsertScopedThread(next, change.thread);
    if (next.currentThread?.thread.id === change.thread.id) next.currentThread.thread = clone(change.thread);
    ...
    if (change.reason === "deleted" && next.currentThread?.thread.id === change.threadId) next.currentThread = null;
    refreshScopeCounts(next);
  } else if (change.kind === "turnUpserted" && next.currentThread?.thread.id === change.turn.threadId) {
    ...
    if (next.currentThread.turns.length > next.limits.turnWindowMax) next.currentThread.turns.splice(0, next.currentThread.turns.length - next.limits.turnWindowMax);
    next.currentThread.thread.headTurnId = next.currentThread.turns.at(-1)?.id ?? next.currentThread.thread.headTurnId;
  } else if (change.kind === "pendingRequestOpened") {
    const index = next.pendingRequests.findIndex((request) => request.id === change.request.id && request.generation === change.request.generation);
    ...
    next.pendingRequests = next.pendingRequests.filter((request) => request.id !== change.requestId || request.generation !== change.generation);
...
export function compareV2Watermarks(left: string, right: string): number {
  ...
  return normalizedLeft.length === normalizedRight.length ? normalizedLeft.localeCompare(normalizedRight) : normalizedLeft.length - normalizedRight.length;
}
...
function assertSnapshotCoverage(snapshot: V2SnapshotFrame): void {
  ...
  for (const [name, threads, scope, archived] of partitions) {
    if (threads.length !== scope.returned || scope.returned > scope.limit || threads.some((thread) => thread.archived !== archived)) {
...
function upsertScopedThread(projection: V2Projection, thread: V2ThreadSummary): void {
  ...
  const entry: V2CatalogEntry = { thread, coverage: limit === 0 ? "outsideCurrentScope" : "current" };
  ...
  enforcePartitionLimit(projection, !thread.archived, (!thread.archived ? projection.scope.archived : projection.scope.active).limit);
  refreshScopeCounts(projection);
}

function enforcePartitionLimit(projection: V2Projection, archived: boolean, limit: number, preferredId?: string): void {
  const current = projection.catalog.filter((entry) => entry.coverage === "current" && entry.thread.archived === archived);
  if (current.length <= limit) return;
  const preferredIsCurrent = preferredId !== undefined && current.some((entry) => entry.thread.id === preferredId);
...
function refreshScopeCounts(projection: V2Projection): void {
  projection.scope.active.returned = projection.catalog.filter((entry) => entry.coverage === "current" && !entry.thread.archived).length;
  projection.scope.archived.returned = projection.catalog.filter((entry) => entry.coverage === "current" && entry.thread.archived).length;
}
...
function appendInvalidation(projection: V2Projection, invalidation: V2SemanticInvalidation): void {
  ...
  if (projection.invalidations.length > projection.limits.queueMaxEvents) {
    projection.invalidations.splice(0, projection.invalidations.length - projection.limits.queueMaxEvents);
  }
}
...
export function retainV2ProjectionOutsideCoverage(projection: V2Projection): V2Projection {
  return {
    ...clone(projection),
    catalog: projection.catalog.map((entry) => ({ ...clone(entry), coverage: "outsideCurrentScope" })),
    scope: {
      active: { ...projection.scope.active, returned: 0 },
      archived: { ...projection.scope.archived, returned: 0 },
    },
    currentThread: null,
    pendingRequests: [],
  };
